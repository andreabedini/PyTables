...
cdef inline int less_than(any_type* a, any_type* b) nogil:
    if any_type in floating_type:
        return a[0] < b[0] or (b[0] != b[0] and a[0] == a[0])
    else:
        return a[0] < b[0]


cdef void _keysort(any_type* start1, char* start2, size_t elsize2, size_t n) nogil:
    cdef any_type *pl = start1
    cdef any_type *pr = start1 + (n - 1)
    ...
    cdef any_type* stack[PYA_QS_STACK], **sptr = stack
    cdef char* istack[PYA_QS_STACK], **isptr = istack
    cdef size_t stack_index = 0

    cdef any_type *pm, *pi, *pj, *pt
    cdef char *ipm, *ipi, *ipj, *ipt
...
cdef void _keysort_string(char* start1, size_t ss, char* start2, size_t ts, size_t n) nogil:
    ...
    cdef char* stack[PYA_QS_STACK], **sptr = stack
    cdef char* istack[PYA_QS_STACK], **isptr = istack
    cdef size_t stack_index = 0

    cdef char *pm, *pi, *pj, *pt
    cdef char *ipm, *ipi, *ipj, *ipt
